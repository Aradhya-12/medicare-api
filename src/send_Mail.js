...
const Notification_of_buyers =  function(email_List, buyers_email, buyers_cnt) {
  var mailOptions = {
      ...
      text: 'Its to inform you that a buyer with' + buyers_email.toString() + ' requires ' + buyers_cnt.toString() + ' ventilator.'
    }
  transporter.sendMail(mailOptions, function(error, info){
if (error) {
  console.log(error);
...
const Notification_of_sellers =  function(email_List, seller_email, seller_cnt) {
  var mailOptions = {
      from: 'limitedmedicare@gmail.com',
      to: email_List,
      subject: 'Notification for ventilators availabity',
      text: 'Its to inform you that a seller with' + seller_email.toString() + ' has ' + seller_cnt.toString() + ' ventilators. If you require ventilators kindly contact this seller'
    }
  transporter.sendMail(mailOptions, function(error, info){
if (error) {
  console.log(error);
